...
class RpcPublisher:
    ...
    async def close(self) -> None:
        if self.connection:
            ...
            logger.info(
                "RPC Publisher: соединение закрыто, очередь '%s'.",
                self.response_queue.name,
            )
